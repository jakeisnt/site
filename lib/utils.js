// determine whether the user's device is mobile
// note: usually not a good idea to use this. instead, use media queries to promote responsiveness!
// reference: https://developer.mozilla.org/en-us/docs/web/http/browser_detection_using_the_user_agent
function isMobile() {
  if(sessionStorage.desktop) return false;
  if(sessionStorage.mobile) return true;
...
// determine whether the device has a touchscreen
// NOT the same as determining whether the device is mobile, though that is a good fallback
// make sure to make your code as dynamic as possible: they can always flip their device!!!
// reference: https://developer.mozilla.org/en-us/docs/web/http/browser_detection_using_the_user_agent
...
// introduce a new scope so we don't have to gensym exposed names (lol js)
{
  const exposedNames = { isMobile, hasTouchScreen };

  // If there is a global `window` object, bind API names to it.
  if (typeof window === 'object') {
    window.Utils = exposedNames;
  }
