function addContents(text, elem) {
  elem.innerText += text;
  return { text: text };
}
...
function createElement(createFn, setAttrFn, createChildString, name, attrs, ... children) {
  const elem = createFn(name);

  attrs && assignAttributes(elem, attrs);

    const childNodes = children && children.map(childFn => (typeof childFn === "string" ? createChildString(childFn, node) : childFn(node)));
}
...
function create(name, attrs, ... children) {
  return (parent) => {
    ...
    const childNodes = children && children.map(childFn => {
      return (typeof childFn === "string" ? addContents(childFn, elem) : childFn(elem))
      });
...
function svg(name, attrs, ...children) {
  return parent => {
    ...
    const childNodes = children && children.map(childFn => (typeof childFn === "string" ? svgString(childFn, node) : childFn(node)));
