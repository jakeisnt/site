...
function svg(name, attrs, ...children) {
  return parent => {
    ...
    attrs && Object.keys(attrs).forEach(attrName => svg.setAttribute(attrName, attrs[attrName]));
    const childNodes = children && children.map(childFn => (typeof childFn === "string" ? addContents(svg, childFn) : childFn(svg)));

    // add the element to the DOM
    (parent ?? document.body).appendChild(svg);

    return {...attrs, node: svg, children: childNodes };
  };
}
...
{
const exposedNames = { svg, css, create, mixin };
